    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            async function loadOrders() {
                ...
                try {
                    const res = await fetch(`get_orders.php?history=true&search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}`);
                    ...
                    if (data.status !== "success" || data.data.length === 0) {
                        ...
                        <div class="text-center text-muted py-4">
                            No order history. Place your order first upon checkout.
                        </div>
                    `;
                        return;
                    }
